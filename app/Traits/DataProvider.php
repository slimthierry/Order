...
/**
    ...
    public function delete($query, $shouldMassDelete = false)
    {
        if (is_array($query)) {
            return $this->repository->resetScope()->pushFilter(
                app(\App\Services\Contracts\Data\IModelFilter::class)->setQueryFilters(
                    $query
                )
    ...
    public function get($query = [], $columns = array('*'), $relationQuery = false, $shouldPaginate = false, $limit = null)
    {
        ...
        return $shouldPaginate ?  $this->repository->resetScope()->pushFilter(
            app(\App\Services\Contracts\Data\IModelFilter::class)
                ->setQueryFilters(is_null($query) ? [] : $query)
        )->{$relationFn}($relationQuery)->paginate($limit) : new \App\Services\Contracts\Data\DataProviderQueryResult(
            $this->repository->resetScope()->pushFilter(
                app(\App\Services\Contracts\Data\IModelFilter::class)->setQueryFilters($query)
     ...
     *
     * @param array|\App\Services\Contracts\Data\IDataProviderHandlerParams $params
     * @return void
     */
    protected function parseProviderUpdateHandlerParams($params)
    {
        $value = $params instanceof \App\Services\Contracts\Data\IDataProviderHandlerParams ? $params->getParams() : (is_array($params) ? $params : []);
